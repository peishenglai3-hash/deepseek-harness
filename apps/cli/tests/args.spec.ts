...
describe('parseDshArgs', () => {
  it('routes profile boots and the web/ide aliases, handing the rest to the app', () => {
    ...
    expect(parse(['ide'])).toEqual({ mode: 'profile', profile: 'ide', patches: [], args: [] })
    expect(parse(['ide', '--patch', 'ide.yml']))
      .toEqual({ mode: 'profile', profile: 'ide', patches: ['ide.yml'], args: [] })
  })
  ...
  it('ends the launcher flags at the first token it does not own', () => {
    ...
    expect(parse(['ide', '--host', '127.0.0.1', '--port', '8080', '--dev']))
      .toEqual({ mode: 'profile', profile: 'ide', patches: [], args: ['--host', '127.0.0.1', '--port', '8080', '--dev'] })
  ...
  it('routes profile and web config dumps', () => {
    ...
    expect(parse(['web', '--dump-default-config']))
      .toEqual({ mode: 'dump-config', profile: 'web', defaultOnly: true, patches: [] })
    expect(parse(['ide', '--dump-config']))
      .toEqual({ mode: 'dump-config', profile: 'ide', defaultOnly: false, patches: [] })
    expect(parse(['ide', '--dump-default-config']))
      .toEqual({ mode: 'dump-config', profile: 'ide', defaultOnly: true, patches: [] })
  })
  ...
  it('rejects missing profile, removed flags, and contradictory inputs', () => {
    ...
    expect(exitCode(['--profile', 'x', 'ide'])).toBe(1)
    expect(exitCode(['web', '--dump-config', '--dump-default-config'])).toBe(1)
    expect(exitCode(['ide', '--dump-config', '--dump-default-config'])).toBe(1)
    expect(exitCode(['web', '--dump-default-config', '--patch', 'w.yml'])).toBe(1)
    expect(exitCode(['ide', '--dump-default-config', '--patch', 'i.yml'])).toBe(1)
    expect(exitCode(['web', '--patch='])).toBe(1)
    expect(exitCode(['ide', '--patch='])).toBe(1)
